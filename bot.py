...
async def help_command(update: telegram.Update,
    ...
    help_message = f"Привет {user.mention_html()}, как дела??\n\n"
    help_message += "Я Suprebot, ваш виртуальный помощник.\n"
    help_message += "Мы можем общаться с помощью текста или голоса.\n\n"
    help_message += "Используйте /read [текст], если хотите, чтобы я прочитал вам текст.\n\n"
    help_message += "О чем мы будем говорить сегодня? \U0001F916"  # robot face emoji
...
async def read_command(update: telegram.Update,
    ...
    if len(text) <= 0:
        no_text_message = "Введите текст после /read!!! \U0001F620"
